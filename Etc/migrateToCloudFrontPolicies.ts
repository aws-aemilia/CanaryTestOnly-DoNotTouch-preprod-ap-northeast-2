 ...
 */
const PROD_FRONT_END_RESOURCE_HANDLER_PREFIX =
  "AemiliaWarmingPool-FrontEndResourceWarmingPool-";
const LOCAL_FRONT_END_RESOURCE_HANDLER_PREFIX =
  "sam-dev-jffranzo-AemiliaW-FrontEndResourceWarmingP-9VZUVO2NGPVJ";
...
const migrateRegion = async (
  ...
  console.log(
    `updating env vars with cloudFront policy IDs for function: ${functionName}`
  );
...
const main = async () => {
  const args = await yargs(process.argv.slice(2))
    ...
    .option("cachPolicyId", {
      describe: "cachPolicyId to set",
      type: "string",
    })
    .option("originRequestPolicyId", {
      describe: "originRequestPolicyId to set",
      type: "string",
    })
    .option("region", {
  ...
  if (!region) {
    throw new Error(
      "no region was provided. please specifcy which region you want to migrate"
    );
  }

  const migrateAccount =
    stage !== "local"
      ? controlPLaneAccounts.find((account) => account.region === region)
      : { accountId: "120804186529" };
  if (!migrateAccount) {
    throw new Error(`cannot find account for region ${region}`);
  }
  console.log(
    `Action: ${action} Stage: ${stage} Region: ${region} AccountId: ${migrateAccount.accountId}`
  );
  ...
  if (!functionName) {
    throw new Error(
      "failed to get funcion name for the FrontEndResourceHandler. Cannot continue with the script."
    );
  }
  ...
  switch (action) {
    case "migrateRegion":
      if (!cachPolicyId || !originRequestPolicyId) {
        throw new Error(
          `cachPolicyId and originRequestPolicyId need to be set.`
        );
      }
      await migrateRegion(
        lambda,
