...
const listWebhooks = async (gitClient: Octokit, owner: string, repo: string): Promise<Webhook[]> => {
...
const deleteWebhook = async (gitClient: Octokit, owner: string, repo: string, webhook: Webhook) => {
...
const main = async () => {
    const args = await yargs(process.argv.slice(2))
        .usage(
            `
      Deletes GitHub webhooks
      `
        )
        .option("githubToken", {
            describe: "token for your github account. must have at least delete repo permissions",
            type: "string",
            demandOption: true
        })
        .option("repo", {
            describe:
                "repo to delete webhooks for",
            type: "string",
            demandOption: true
        })
        .option("region", {
    ...
    if (!region) {
        ...
        const amplifyRegionWebhooks = filterAmplifyWebhooksByRegion(amplifyWebhooks, region)
        if (amplifyRegionWebhooks.length === 0) {
            console.log(`there are no amplify webhooks for repo ${repo} and region: ${region}`);
            return;
        }
