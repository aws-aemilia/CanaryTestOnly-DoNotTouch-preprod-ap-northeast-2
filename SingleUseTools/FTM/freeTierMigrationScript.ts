...
const dlqHasMessages = async (): Promise<boolean> => {
  const checkDLQResponse = await sqs.send(
  ...
  return (
    (checkDLQResponse &&
      checkDLQResponse.Messages &&
      checkDLQResponse.Messages.length > 0) ||
    false
  );
};
...
const getAccountIdIdxToStartProcessing = (): number => {
  ...
  if (fs.existsSync(LAST_PROCESSED_FILE_PATH)) {
    console.log(
      `Found ${LAST_PROCESSED_FILE_PATH} file to start processing from.`
    );
    const account = JSON.parse(
      fs.readFileSync(LAST_PROCESSED_FILE_PATH, "utf8")
    );
...
const main = async () => {
  ...
  if (await dlqHasMessages()) {
    throw new Error(
      "Found messages in the FreeTierMigrationDLQ. Either redrive or purge them before rerunning this script."
    );
  }
  ...
  if (!fs.existsSync(inputAccountIdsFilePath))
    throw new Error(
      `Expected to find input data file "${inputAccountIdsFilePath}" but the path does not exist.`
    );
    ...
    try {
      const message = new SendMessageCommand({
        ...
        MessageBody: JSON.stringify(
          buildAccountMigrationMessage(ACCOUNT_BEING_PROCESSED)
        ),
      ...
      console.log(
        `progress: ${((i / accountIds.length) * 100.0).toFixed(2)} %`
      );
    } catch (err) {
      console.warn(
        `Failed to send message for accountId ${ACCOUNT_BEING_PROCESSED}`
      );
      console.warn(err);
      throw err;
    }
...
const runShutDownSequence = () => {
  ...
  const data = {
    accountId: ACCOUNT_BEING_PROCESSED,
    accountIdIdx: ACCOUNT_IDX_BEING_PROCESSED,
  };
