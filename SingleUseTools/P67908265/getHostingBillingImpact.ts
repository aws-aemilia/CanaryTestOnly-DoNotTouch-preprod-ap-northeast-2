...
import {
  ...
  startAthenaQuery,
  waitForAthenaQuery,
} from "../../Commons/utils/athena";
import {
  getRedshiftQueryResults,
  startRedshiftQuery,
  waitForRedshiftQuery,
} from "../../Commons/utils/redshift";
...
const formatJsonToCsv = (accountMap: AccountMap): string => {
  let str =
    "accountId, usage (GB), billedRevenue, athenaUsage(GB), percentOverBilled\n";
  for (const account of Object.keys(accountMap)) {
    const overBillPercent = (
      (accountMap[account].usage - accountMap[account].athenaUsage) /
      accountMap[account].athenaUsage
    ).toFixed(2);
...
const main = async () => {
  const redshiftClient = new RedshiftDataClient({
    ...
    credentials: getIsengardCredentialsProvider(
      BUSINESS_METRICS_ACCOUNT,
      "RedshiftQueryRole"
    ), // todo need to create this RedshiftQueryRole
  });
  const accountBillMap: AccountMap = readAccountCSVFile(
    "billing_acct_mismatch"
  );
  ...
  for (const account of accounts) {
    ...
    try {
      ...
      const regions = Array.from(
        new Set(
          appMap[account].map((entry) => {
            return entry.region;
          })
        )
      );
      console.log(
        `[${account}] ${appsForAccount.length} apps found across ${regions.length} region(s)`
      );

      for (const region of regions) {
        // 3) init an Athena client in the region
        console.log(
          `[${account}][${region}] running athena query to get bytes usage in region ${region}`
        );
        const controlPLaneAccounts = (await controlPlaneAccounts()).filter(
          (acc) => acc.stage === "prod"
        );
        const serviceAccountId = controlPLaneAccounts.find(
        ...
        const athenaClient = new AthenaClient({
          ...
          credentials: getIsengardCredentialsProvider(
            ...
            "AthenaReportGeneration"
          ), // Todo Not all our accounts have this role set up.
        });
      ...
      fs.writeFileSync(
        path.join(__dirname, `/account_impact.csv`),
        formatJsonToCsv(accountBillMap)
      );
    }
  }
};
