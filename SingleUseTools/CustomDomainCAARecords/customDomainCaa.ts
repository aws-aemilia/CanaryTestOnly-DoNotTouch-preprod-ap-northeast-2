...
import { lookupCustomerAccountId, paginateDomains } from "../../Commons/dynamodb";
 ...
 */
async function listDomains(documentClient: DynamoDBDocumentClient, stage: Stage, region: RegionName) {
  const domainName = "domainName", appId = "appId", status = "#s";
  ...
  const isAvailable = (item: Record<string, any>) => item["status"] === "AVAILABLE";
  const extractFields = (item:  Record<string, any>): DomainInfo => ({ domainName: item[domainName], appId: item[appId] });

  for await (const page of paginatedDomains) {
    const retrievedItems = page.Items?.filter(isAvailable).map(extractFields) ?? [];
...
async function getMisconfiguredDomains(
  ...
  for (const domain of domains) {
    ...
    if (cas?.length > 0 && !hasAmazonCAs(cas)) {
      domain.customerAccountId = await lookupCustomerAccountId(
        documentClient,
        stage,
        region,
        domain.appId
...
async function main() {
  ...
  for (const account of accounts) {
    ...
    const domains = await listDomains(documentClient, account.stage as Stage, account.region);
    let misconfiguredDomains = await getMisconfiguredDomains(documentClient, account.stage as Stage, account.region, domains);

    const csv = stringify(misconfiguredDomains.map((item: DomainInfo) => [account.region, item.domainName, item.appId, item.customerAccountId]));
