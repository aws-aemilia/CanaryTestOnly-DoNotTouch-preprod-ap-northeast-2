...
(async function () {
  ...
  const allAccountsWithAirportCodes: AmplifyAccount[] =
    allAccountResults.flat();

  const newSubdomain = await getNewSubdomainFromUserInput(
    allAccountsWithAirportCodes
  );
  console.log({ newSubdomain });

  for (const accountWithAirportCode of allAccountsWithAirportCodes) {
    console.log(
      `Getting credentials for region: ${accountWithAirportCode.airportCode}`
    );

    const credentials: Credentials = await getIsengardCredentialsProvider(
      accountWithAirportCode.accountId,
      DANGEROUS_ROLE_ALLOWING_CHANGES_TO_BE_COMMITTED
    )();

    console.log(`Looking up existing zone`);
    const existingHostedZone = await lookupIamAliveByAccountWithAirportCode({
    ...
    console.log("------------------------------------\n\n");
    console.log(
      `Are you sure you want to add the new subdomain: ${newSubdomain} to the following:`
    );
    ...
    const newHostedZoneWithNameServers = await createNewHostedZoneForName({
      accountWithAirportCode,
      credentials,
      newSubdomain,
    });
    console.log(
      `new HostedZone created: ${newHostedZoneWithNameServers.hostedZone.Name}`
    );

    console.log(
      `Adding new subdomain: ${newHostedZoneWithNameServers.hostedZone.Name} to existing HostedZone`
    );
    ...
    console.log(
      `Success for stage: ${accountWithAirportCode.stage} region: ${accountWithAirportCode.airportCode}\n\n------------------------------\n\n`
    );
  }
