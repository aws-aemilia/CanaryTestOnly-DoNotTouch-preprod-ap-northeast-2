...
export async function lookupIamAliveByAccountWithAirportCode({
  ...
  const dnsName = buildDnsNameFromAccountWithAirportCode(
    accountWithAirportCode
  );
  ...
  if (!response.HostedZones) {
    throw new Error(
      `No Hosted zones were returned for dns name: ${dnsName} in account ${accountWithAirportCode.accountId} in region: ${accountWithAirportCode.region}`
    );
  }

  const hostedZone = response.HostedZones.find(
    (hostedZone) => hostedZone.Name === `${dnsName}.`
  );

  if (!hostedZone) {
    throw new Error(
      `Hosted zone not found for dns name: ${dnsName} in account ${accountWithAirportCode.accountId} in region: ${accountWithAirportCode.region}`
    );
  }
