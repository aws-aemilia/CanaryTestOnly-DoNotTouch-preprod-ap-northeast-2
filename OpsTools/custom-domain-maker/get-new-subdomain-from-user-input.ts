...
export async function getNewSubdomainFromUserInput(allAccountsWithAirportCodes: AmplifyAccount[]): Promise<string> {
...
`);
    ...
    const newSubdomain = await validateResponseWithMessage({
        ...
        errorMessage: 'Subdomains must only consist of lowercase letters, numbers, and hyphens.'
    });

    const newDomains: string[] = [];
    allAccountsWithAirportCodes.forEach(accountWithAirportCode => {
        const iamaliveDomain = buildDnsNameFromAccountWithAirportCode(accountWithAirportCode);
