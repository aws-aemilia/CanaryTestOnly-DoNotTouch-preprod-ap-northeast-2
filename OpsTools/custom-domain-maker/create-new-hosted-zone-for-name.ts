...
export async function createNewHostedZoneForName({
  ...
  if (!response.DelegationSet || !response.DelegationSet.NameServers) {
    throw new Error(
      "Failed with partially created HostedZone - a DelegationSet with NameServers was not returned"
    );
  }

  if (!response.HostedZone) {
    throw new Error(
      "Failed with partially created HostedZone - The HostedZone was not returned with the response"
    );
  }

  const nameServersWithRequiredPeriodEnding =
    response.DelegationSet.NameServers.map((nameServer) => `${nameServer}.`);
