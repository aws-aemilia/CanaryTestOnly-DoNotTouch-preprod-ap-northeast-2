...
async function getAllAccounts(
  accountTypes: AmplifyAccountType[],
  stage: Stage,
  region: Region
) {
  const accounts: AmplifyAccount[] = [];
  for (const accountType of accountTypes) {
    const moreAccounts = await getAccountsLookupFn[accountType]({
      stage,
      region,
    });
...
type NonNullableProps<T> = {
  [P in keyof T]: T[P] extends object
    ? NonNullableProps<T[P]>
    : NonNullable<T[P]>;
};

async function requestQuotaIncrease(
  account: AmplifyAccount,
  request: NonNullableProps<RequestServiceQuotaIncreaseCommandInput>
) {
  ...
  try {
    const getServiceQuotaCommandOutput = await serviceQuotas.getServiceQuota(
      request
    );
    ...
    logger.info(
      `Requesting ${quotaName} limit increase for ${account.email}...`
    );
    ...
    if (currentQuotaValue < request.DesiredValue) {
      ...
      logger.info(
        `...quota is already at or above the desired value, skipping.`
      );
    }
  } catch (e) {
...
async function main() {
  ...
  const { accountTypes, serviceCode, quotaCode, desiredValue } = args;
