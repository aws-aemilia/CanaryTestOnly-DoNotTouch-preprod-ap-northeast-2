...
async function getArgs() {
  return (await yargs(hideBin(process.argv))
    .usage(`Simulate a cache-busting attack by putting access log records corresponding to cache misses on the ${DATA_STREAM_NAME} data stream.`)
    .option("stage", {
...
async function main() {
  const { stage, region, batches, domainId, status, resultType } = await getArgs();
