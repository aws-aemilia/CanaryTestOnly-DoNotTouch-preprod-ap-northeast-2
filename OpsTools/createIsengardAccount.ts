...
const cutTicketsLambdaLimitIncrease = async (
  createdAccount: AmplifyAccount,
  type: AmplifyAccountType
): Promise<void> => {
  ...
  console.log(
    `concurrency increase: https://t.corp.amazon.com/${concurrencyTicket}`
  );
  ...
  console.log(
    `code storage increase: https://t.corp.amazon.com/${storageTicket}`
  );
};
...
`
  ...
  switch (type) {
    case "computeServiceControlPlane":
      ...
      const computeAccount = await computeServiceControlPlaneAccount(
        stage,
        region
      );
