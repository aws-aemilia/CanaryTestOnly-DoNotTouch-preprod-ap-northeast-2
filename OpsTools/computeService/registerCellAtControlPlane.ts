...
const activateCellFn = (accountId: string): UpdateCommand => {
    return new UpdateCommand({
        ...
        ConditionExpression: 'attribute_exists(accountId) and #status = :inactive and attribute_exists(amplifyRuntimeLayerArn) and attribute_exists(amplifySharedLibrariesLayerArn)',
        TableName: TABLE_NAME,
    });
}
...
const registerAccount = async(stage: Stage, region:Region, cellNumber:number, action: CellAction) => {
    ...
    console.log(`Working on cell account ${cellAccount.accountId} - ${cellAccount.email}`);
