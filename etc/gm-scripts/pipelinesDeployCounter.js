...
// @grant        GM_addStyle
// @updateURL    https://code.amazon.com/packages/AWSAmplifyTools/blobs/mainline/--/gm-scripts/pipelinesDeployCounter.js?raw=1
    ...
    function getDeployCounts(team) {
        ...
        const req = typeof (GM_xmlhttpRequest) == "function" ? GM_xmlhttpRequest : GM.xmlHttpRequest;
         $.each($("#pipelines_by_teams").children('ul').children('li'), (_, item) => {
            ...
            if (teamName === team) {
                pipelineCount = $(item).children('table').find("tr").length-1;
